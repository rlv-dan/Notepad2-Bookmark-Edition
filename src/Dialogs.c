...
#ifdef BOOKMARK_EDITION
		SetDlgItemText(hwnd,IDC_RLV_TEXT,VERSION_BOOKMARK_ED);
        if (GetDlgItem(hwnd,IDC_RLV_TEXT) == NULL)
		{
          SetDlgItemText(hwnd,IDC_RLV_LINK2,VERSION_BOOKMARK_ED_LINK);
          ShowWindow(GetDlgItem(hwnd,IDC_RLV_LINK2),SW_SHOWNORMAL);
        }
        else
		{
          wsprintf(wch,L"<A>%s</A>",VERSION_BOOKMARK_ED_LINK);
          SetDlgItemText(hwnd,IDC_RLV_LINK,wch);
        }
#endif
          ...
          case NM_RETURN:
            {
              ...
              else if (pnmhdr->idFrom == IDC_RLV_LINK) {
